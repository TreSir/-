...
| 普通存档、落笔检查点、断链回溯、旧版存档迁移与备份恢复 | 多槽位界面 |
...
- 玩家偏好：独立保存，不随新游戏清空
- 当前存档结构：`schema = 2`，内容标识为 `black_page_mvp2`（磁盘封装 `format = 1`）

相同内容标识的 `schema = 1` 存档会在内存中升级为 2，再接受完整校验；读取和迁移不会修改原文件。内容标识不同、未来版本或损坏状态仍会被拒绝。校验包括 Flag 类型、道具数量、单活动案件约束、待结算姓名、日志上限和结局 ID，不进行不完整恢复。

覆盖普通存档前会将可读取的上一份保留为同目录的 `.json.bak`。主存档损坏或校验失败时，“继续游戏”与游戏内读档可以读取通过完整校验的备份，进度会回到上一次保存；损坏的主文件不会在恢复时被覆盖。黑页检查点不使用旧备份回溯，避免退到错误的落笔之前。删除槽位时，主文件和备份会一起删除。
